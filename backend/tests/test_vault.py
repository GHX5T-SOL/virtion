...
class VaultEndpointTests(unittest.TestCase):
    ...
    def test_log_line_never_contains_vault_token(self) -> None:
        log = logging.getLogger("virtion.agent")
    ...
    def test_lookup_tool_registered_in_VIRTION_CUSTOM_TOOLS(self) -> None:
        names = {t["name"] for t in server.VIRTION_CUSTOM_TOOLS}
        self.assertIn("lookup_ehr_history", names)

    def test_vault_never_references_token_in_tool_description(self) -> None:
        tool = next(
            t for t in server.VIRTION_CUSTOM_TOOLS if t["name"] == "lookup_ehr_history"
        )
