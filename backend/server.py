"""
FastAPI backend for the Virtion simulator.

Hosts the Claude Managed Agents proxy (virtion-attending grading) and the
...
POST /agent/...      → Managed Agents proxy (virtion-attending)
...
app = FastAPI(title="Virtion Backend", version="0.2.0")
...
def health():
    ...
    agent_id = os.environ.get("VIRTION_AGENT_ID") or None
    env_id = os.environ.get("VIRTION_ENV_ID") or None
...
#   VIRTION_AGENT_ID    — persisted agent ID (bootstrap returns it the
#                        first time; set it here afterwards to skip
#                        re-creating).
#   VIRTION_ENV_ID      — persisted environment ID (same pattern).
#
# Endpoints:
...
_agent_log = logging.getLogger("virtion.agent")
_agent_log.setLevel(logging.INFO)
if not _agent_log.handlers:
    _h = logging.StreamHandler()
    _h.setFormatter(logging.Formatter("[virtion.agent] %(levelname)s %(message)s"))
...
AGENT_NAME = "virtion-attending"
ENV_NAME = "virtion-attending-env"
...
VIRTION_ATTENDING_SYSTEM_PROMPT = (
...
VIRTION_CUSTOM_TOOLS: list[dict] = [
    {
        "type": "custom",
...
def bootstrap_agent():
    """Idempotent: creates the Virtion attending agent + environment if
    VIRTION_AGENT_ID and VIRTION_ENV_ID aren't set; otherwise returns the
    cached IDs.
    ...
    with _bootstrap_lock:
        ...
        agent_id = os.environ.get("VIRTION_AGENT_ID")
        env_id = os.environ.get("VIRTION_ENV_ID")
        ...
        try:
            ...
            agent = client.beta.agents.create(  # type: ignore[attr-defined]
                name=AGENT_NAME,
                model=AGENT_MODEL,
                system=VIRTION_ATTENDING_SYSTEM_PROMPT,
                tools=[
                    {"type": "agent_toolset_20260401", "default_config": {"enabled": True}},
                    *VIRTION_CUSTOM_TOOLS,
                ],
            )
        ...
        os.environ["VIRTION_AGENT_ID"] = agent.id
        os.environ["VIRTION_ENV_ID"] = env.id
        _agent_log.info(
...
def refresh_agent():
    ...
    Use this whenever you edit ``VIRTION_ATTENDING_SYSTEM_PROMPT`` or
    ``VIRTION_CUSTOM_TOOLS`` so the change takes effect without creating a
    whole new Agent."""
    agent_id = os.environ.get("VIRTION_AGENT_ID")
    if not agent_id:
        raise HTTPException(
            status_code=400,
            detail="VIRTION_AGENT_ID not set. Run /agent/bootstrap first.",
        )
    client = get_anthropic_client()
    try:
...
def create_session(req: CreateSessionRequest):
    ...
    if not agent_id or not env_id:
        raise HTTPException(
            status_code=400,
            detail=(
                "VIRTION_AGENT_ID / VIRTION_ENV_ID not set. Call POST /agent/bootstrap "
    ...
    try:
        session = client.beta.sessions.create(  # type: ignore[attr-defined]
            agent=agent_id,
            environment_id=env_id,
            title=req.title or "virtion training shift",
        )
    except Exception as e:
...
# ───────────────────────────────────────────────────────────────────────────
#
# Separate from the virtion-attending Managed Agent. That agent observes the
...
#     mirrors `.claude/skills/virtion-triage-logic.md` so keep them in sync
#     when either changes.
...
async def voice_token(req: VoiceTokenRequest):
    ...
    # `agents=[RoomAgentDispatch(agent_name="virtion-voice")]` makes dispatch
    ...
    try:
        await lkapi.room.create_room(
            _lkapi.CreateRoomRequest(
                ...
                metadata=metadata,
                empty_timeout=120,
                agents=[_lkapi.RoomAgentDispatch(agent_name="virtion-voice")],
            )
        )
    except Exception as e:
