# Spec — Virtion hackathon submission
...
| Opus 4.7 use | (a) Opus 4.7 on the `virtion-attending` Managed Agent (whole-encounter observer + grader). (b) *Separate* Opus 4.7 direct-inference endpoint `POST /agent/triage/classify` for one-shot ESI triage at ER arrival — two distinct Opus code paths in the same submission. (c) `/loop 20m /virtion-verify-simulation` writing timestamped PASS/FAIL to `verify.log` through the shift. (d) Auto mode for the whole build. |
| Depth | Skills composed (no specialist sub-agents): `virtion-patient-generator`, `virtion-verify-simulation`, `virtion-triage-logic`, `virtion-managed-agent-setup`, `virtion-interview`, `virtion-demo-video`. Keep-thinking log in `docs/evolution.md`. |
...
**Name:** `virtion-attending` — virtual attending physician reviewing the player's decisions in real time.
...
| Skill | Purpose |
|---|---|
| `virtion-interview.md` | Claude asks us questions about the idea before building. |
| `virtion-patient-generator.md` | Generate a new `PatientCase` given a chief complaint + correct diagnosis. |
| `virtion-verify-simulation.md` | Run every `scripts/verify/*.ts`. Fail loud on violation. |
| `virtion-managed-agent-setup.md` | Create/update the Agent, Environment, custom tools; wire the frontend event stream. |
| `virtion-triage-logic.md` | ESI protocol reference — when Claude makes triage calls, this skill scopes the reasoning. |
| `virtion-demo-video.md` | Extract design-system HTML, run Remotion, produce the submission MP4. |

## `/loop` usage

- `/loop 20m /virtion-verify-simulation` — runs verification over the current patient queue and log file. Writes a one-line PASS/FAIL to `verify.log`.
- `/loop 30m /virtion-idea-evolve` — reads `docs/evolution.md`, proposes one sharpening tweak, waits for approval.
...
- **Opus 4.7 use:** TWO distinct code paths on Opus 4.7 — (a) the `virtion-attending` Managed Agent (whole-encounter grading), (b) `POST /agent/triage/classify` direct-inference ESI classifier. Plus `/loop 20m /virtion-verify-simulation` for long-running verification and `/loop 30m /virtion-idea-evolve` for the Keep-Thinking cadence. Auto mode used throughout the build.
...
- **Skills we wrote** (six, in `.claude/skills/`): `virtion-interview`, `virtion-patient-generator`, `virtion-verify-simulation`, `virtion-triage-logic`, `virtion-managed-agent-setup`, `virtion-demo-video`. None of them are specialist sub-agents — each is a skill Claude composes as needed. Plus two slash commands in `.claude/commands/` (`virtion-verify-simulation` for the `/loop` runner, `virtion-idea-evolve` for the evolution-log cadence).
