...
[backend/server.py](../../../backend/server.py) under `VIRTION_ATTENDING_SYSTEM_PROMPT`.
...
the JSON schema in `backend/server.py:VIRTION_CUSTOM_TOOLS`). Required fields:
...
| File | Role |
|---|---|
| [backend/server.py](../../../backend/server.py) | `VIRTION_ATTENDING_SYSTEM_PROMPT` (DEBRIEF MODE section) + `VIRTION_CUSTOM_TOOLS[render_case_evaluation]` JSON schema |
...
- Treating an unresolved citation in the UI as an agent bug. It usually means
  ...
  recommendation in `guidelines.ts` (via virtion-guideline-curator) or remove the
