---
description: Run deterministic simulator invariants and append PASS/FAIL to verify.log. Safe for `/loop 20m /virtion-verify-simulation`.
---

# `/virtion-verify-simulation`

Long-running verification tick. Designed to be fired every 20 minutes
via `/loop 20m /virtion-verify-simulation` so the simulator's invariants
...
- **FAIL firing:** `verify.log` gets `<ISO8601> FAIL  <count>`, exit
  ...
  2. Fix the underlying data/code — NOT the verify script. The skill
     [virtion-verify-simulation](../skills/virtion-verify-simulation.md) spells
     out the rule.
