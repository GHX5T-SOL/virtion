...
test('backend/server.py exposes every name from CUSTOM_TOOL_NAMES', () => {
  ...
  for (const name of CUSTOM_TOOL_NAMES) {
    ...
    assert.match(
      py,
      re,
      `backend/server.py is missing a "name": "${name}" entry in VIRTION_CUSTOM_TOOLS`,
    );
  }
});
...
test('backend/server.py does not advertise tools missing from the frontend registry', () => {
  const py = readFileSync(BACKEND_SERVER, 'utf8');
  // Extract every "name": "..." inside VIRTION_CUSTOM_TOOLS. We scope to the
  // list body so we don't pick up unrelated "name": "..." pairs (e.g.
  // AGENT_NAME / ENV_NAME at the top).
  const listStart = py.indexOf('VIRTION_CUSTOM_TOOLS: list[dict] = [');
  assert.ok(listStart >= 0, 'VIRTION_CUSTOM_TOOLS list not found in server.py');
  ...
  assert.ok(
    ...
    'VIRTION_CUSTOM_TOOLS list terminator not found',
  );
