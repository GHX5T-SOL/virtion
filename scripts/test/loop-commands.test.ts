...
test('virtion-verify-simulation command file exists, has frontmatter, and points at the runner', () => {
  const cmdPath = resolve(REPO_ROOT, '.claude/commands/virtion-verify-simulation.md');
...
test('virtion-idea-evolve command file exists and enforces draft-only behavior', () => {
  const cmdPath = resolve(REPO_ROOT, '.claude/commands/virtion-idea-evolve.md');
  assert.ok(existsSync(cmdPath), `${cmdPath} missing`);
  const body = readFileSync(cmdPath, 'utf8');
  assert.match(body, /^---\s*\n[\s\S]+?\n---/, 'missing YAML frontmatter');
...
test('verify-loop script is not referenced by accident outside its command', () => {
  ...
  const allowlist = new Set([
    '.claude/commands/virtion-verify-simulation.md',
    'scripts/loop/verify-loop.ts',
