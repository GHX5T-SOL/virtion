 ...
 * check returns violations. Used by the virtion-verify-simulation skill and
...
import { verifyPatientIdentities } from './patient-identities.ts';
...
const checks: Array<{ name: string; run: () => Violation[] }> = [
  ...
  { name: 'patient-identities', run: verifyPatientIdentities },
];
