...
const NODES: NodeDef[] = [
  ...
  },
  {
    ...
    details: {
      ...
      files: [
        { path: 'backend/server.py', label: 'VIRTION_ATTENDING_SYSTEM_PROMPT + VIRTION_CUSTOM_TOOLS' },
        { path: 'src/agents/customTools.ts', label: 'Zod mirror of the tool schema' },
        { path: '.claude/skills/virtion-attending-debrief/SKILL.md', label: 'reference skill' },
      ],
      hardRules: [
  ...
  // ── AUTHORING row (all nodes y=540-720, height 180) ────────────
  {
    ...
    details: {
      ...
      files: [
        { path: '.claude/skills/virtion-guideline-curator/society-whitelist.json', label: 'allowlist + blocklist' },
      ],
    },
  },
  {
    ...
    w: 220,
    h: 180,
    title: '/virtion-guideline-curator',
    ...
    details: {
      ...
      files: [
        { path: '.claude/skills/virtion-guideline-curator/SKILL.md', label: 'SKILL.md (full instructions)' },
        { path: '.claude/skills/virtion-guideline-curator/society-whitelist.json', label: 'allowlist sidecar' },
      ],
      hardRules: [
  ...
  },
  {
    ...
    details: {
      ...
      files: [
        { path: '.claude/skills/virtion-rubric-author/SKILL.md', label: 'SKILL.md' },
        { path: '.claude/skills/virtion-rubric-author/rubric-brief.schema.json', label: 'optional input contract' },
...
const TABS: Array<{ key: TabKey; label: string; sub: string }> = [
  ...
  { key: 'agent',   label: 'The agent',    sub: 'virtion-attending in detail' },
];
...
const CASE_PIPELINE: PipelineStep[] = [
  ...
  { kind: 'agent', title: '/virtion-guideline-curator', sub: 'WebFetch + verbatim extraction. /loop 7d.', badge: 'skill' },
  { kind: 'data', title: 'guidelines.ts', sub: 'Citation registry — 22 verbatim recommendations.' },
  { kind: 'agent', title: '/virtion-patient-generator', sub: 'Authors the PatientCase shape from condition + variant brief.', badge: 'skill' },
  { kind: 'data', title: 'polyclinicPatients.ts', sub: '246 cases across 11 specialties. Demographics, vitals, anamnesis, gold-standard dx.' },
  { kind: 'agent', title: '/virtion-rubric-author', sub: "Adds rubric: { ... } to hero cases. Cite-don't-invent.", badge: 'skill' },
...
// ────────────────────────────────────────────────────────────────────
// AGENT TAB — virtion-attending Managed Agent deep-dive
// ────────────────────────────────────────────────────────────────────
...
const SKILLS_AROUND = [
  { name: 'virtion-attending-debrief',   role: "Reference for the live agent's DEBRIEF MODE contract — files, hard rules, smoke tests." },
  { name: 'virtion-guideline-curator',   role: 'Fills / refreshes the registry from authoritative sources. Designed for /loop 7d.' },
  { name: 'virtion-rubric-author',       role: 'Authors a CaseRubric per case with citation discipline.' },
  { name: 'virtion-managed-agent-setup', role: 'Bootstrap, refresh, custom-tool maintenance.' },
  { name: 'virtion-patient-generator',   role: 'Authors new PatientCase entries from condition + variant brief.' },
];
