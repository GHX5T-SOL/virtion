# Virtion — Claude Code project notes
...
- `src/agents/managedAgent.ts` / `customTools.ts` / `eventStreamRenderer.tsx` — Claude Managed Agents integration (the attending physician). See `.claude/skills/virtion-managed-agent-setup.md`.
...
- `npm run verify` — deterministic invariant checks on `src/data/*` (see `.claude/skills/virtion-verify-simulation.md`). Run after every data/type/store edit.
